...
@php
    $lang = \App\Models\User\Language::query()->where('user_id', \Illuminate\Support\Facades\Auth::guard('web')->user()->id)->where('code', request()->language)->first();
        ...
        <div class="row justify-content-center align-items-center mb-1">
            <div class="col-12">
                <div class="alert border-left border-primary text-dark">
                    <strong class="text-danger">{{__("Edit Buttons are disabled, because you have more category ($count) than your current package limit ($category_limit).")}}</strong><br>
                </div>
            </div>
        </div>
    @endif
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            @if($count > $category_limit)
                                <a type="button"
                                   class="btn btn-primary btn-sm float-lg-right float-left disabled-btn" disabled><i
                                        class="fas fa-plus"></i> {{ __('Add Category') }}
                                </a>
                            @else
                                    ...
                                    <table class="table table-striped mt-3" id="basic-datatables">
                                        ...
                                        @foreach ($categories as $category)
                                            <tr>
                                                    ...
                                                    <td>
                                                        <form id="featuredForm-{{ $category->id }}"
                                                            ...
                                                            <select
                                                                ...
                                                                <option
                                                                    value="1" {{ $category->is_featured == 1 ? 'selected' : '' }}>
                                                                    {{ __('Yes') }}
                                                                </option>
                                                                <option
                                                                    value="0" {{ $category->is_featured != 1 ? 'selected' : '' }}>
                                                                    {{ __('No') }}
                                                                </option>
                                                            </select>
                                                        </form>
                                                ...
                                                <td>
                                                    @if($count > $category_limit)
                                                        <a type="button" class="btn btn-secondary btn-sm mr-1 disabled-btn" disabled>
