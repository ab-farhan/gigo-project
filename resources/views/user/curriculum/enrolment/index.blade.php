                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 offset-lg-2">
                            ...
                            <form class="float-right ml-3" action="{{ route('user.course_enrolments') }}" method="GET">
                                <input type="hidden" name="status" value="{{ !empty(request()->input('status')) ? request()->input('status') : '' }}">
                                <div class="row">
                                    <div class="col-lg-6">
                                    ...
                                    <div class="col-lg-6 pl-0">
                                        <input name="course" type="text" class="form-control" placeholder="Search By Course Name" value="{{ !empty(request()->input('course')) ? request()->input('course') : '' }}">
                                    </div>
                                </div>
                            ...
                            <form id="searchByStatusForm" class="float-right d-flex flex-row align-items-center"
                                  ...
                                  <input type="hidden" name="order_id" value="{{ !empty(request()->input('order_id')) ? request()->input('order_id') : '' }}">
                                  <input type="hidden" name="course" value="{{ !empty(request()->input('course')) ? request()->input('course') : '' }}">
                                ...
                                <select class="form-control" name="status"
                                    ...
                                    <option
                                        value="completed" {{ request()->input('status') == 'completed' ? 'selected' : '' }}>
                                        {{ __('Completed') }}
                                    </option>
                                    <option
                                        value="pending" {{ request()->input('status') == 'pending' ? 'selected' : '' }}>
                                        {{ __('Pending') }}
                                    </option>
                                    <option
                                        value="rejected" {{ request()->input('status') == 'rejected' ? 'selected' : '' }}>
                                        {{ __('Rejected') }}
                                    </option>
                                </select>
                            </form>
                                    ...
                                    <table class="table table-striped mt-3">
                                        ...
                                        @foreach ($enrolments as $enrolment)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('front.user.course.details', [$user->username, 'slug' => $slug]) }}" target="_blank">
                                                        {{ strlen($title) > 35 ? mb_substr($title, 0, 35, 'utf-8') . '...' : $title }}
                                                    </a>
                                                </td>

                                                <td><a href="{{route('user.user_details', [$enrolment->customer->id])}}">{{ $enrolment->customer->username }}</a></td>
                                                ...
                                                <td>
                                                    ...
                                                    @elseif ($enrolment->payment_status == 'free')
                                                        <h2 class="d-inline-block"><span
                                                                class="badge badge-primary">{{ __('Free') }}</span>
                                                        </h2>

                                                    @elseif ($enrolment->gateway_type == 'offline')
                                                        <form id="paymentStatusForm-{{ $enrolment->id }}"
                                                            ...
                                                            <select
                                                                ...
                                                                <option
                                                                    value="completed" {{ $enrolment->payment_status == 'completed' ? 'selected' : '' }}>
                                                                    {{ __('Completed') }}
                                                                </option>
                                                                <option
                                                                    value="pending" {{ $enrolment->payment_status == 'pending' ? 'selected' : '' }}>
                                                                    {{ __('Pending') }}
                                                                </option>
                                                                <option
                                                                    value="rejected" {{ $enrolment->payment_status == 'rejected' ? 'selected' : '' }}>
                                                                    {{ __('Rejected') }}
                                                                </option>
                                                            </select>
                                                        </form>
                                                ...
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                            ...
                                                            <a href="{{ \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_ENROLLMENT_INVOICE,$enrolment->invoice,$userBs) }}"
                                            ...
                                            @includeIf('user.curriculum.enrolment.show-attachment')
                                        @endforeach
                                        </tbody>
                                    </table>
                                </div>
