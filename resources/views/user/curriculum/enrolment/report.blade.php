      ...
      <div class="card">
        <div class="card-header p-1">
            <div class="row">
              ...
              <div class="col-lg-2">
                <form action="{{route('user.course_enrolments.export')}}" class="form-inline justify-content-end">
                    <div class="form-group">
                    ...
                    <tbody>
                      @foreach ($enrolments as $key => $enrolment)
                        <tr>
                          ...
                          @php
                              $courseInfo = $enrolment->course->courseInformation()->where('language_id', $deLang->id)->select('title', 'slug')->first();
                              $title = $courseInfo->title;
                              $slug = $courseInfo->slug;
                          @endphp
                          <td><a href="{{route('front.user.course.details', [Auth::guard('web')->user()->username, 'slug' => $slug])}}" target="_blank">{{strlen($title) > 35 ? mb_substr($title, 0, 35, 'utf-8') . '...' : $title}}</a></td>
                        ...
                        {{-- Receipt Modal --}}
                        <div class="modal fade" id="receiptModal{{$enrolment->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <img src="{{asset('assets/front/receipt/' . $enrolment->receipt)}}" alt="Receipt" width="100%">
                                </div>
