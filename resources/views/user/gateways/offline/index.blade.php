         ...
         <div class="card-header">
            <div class="row">
               ...
               <div class="col-lg-6 mt-2 mt-lg-0">
                  <a href="#" class="btn btn-primary float-lg-right float-left btn-sm" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{__('Add Gateway')}}</a>
               </div>
            </div>
         </div>
         <div class="card-body">
                     ...
                     <table class="table table-striped mt-3" id="basic-datatables">
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td>
                                <form id="productForm{{$ogateway->id}}" class="d-inline-block" action="{{route('user.offline.status')}}" method="post">
                                @csrf
                              ...
                              <td>
                                <a class="btn btn-secondary btn-sm editbtn" href="#editModal" data-toggle="modal" data-ogateway_id="{{$ogateway->id}}" data-name="{{$ogateway->name}}" data-short_description="{{$ogateway->short_description}}" data-instructions="{{replaceBaseUrl($ogateway->instructions)}}" data-is_receipt="{{$ogateway->is_receipt}}" data-serial_number="{{$ogateway->serial_number}}">
                                 ...
                                 <form class="deleteform d-inline-block" action="{{route('user.offline.gateway.delete')}}" method="post">
                                    @csrf
                                    <input type="hidden" name="ogateway_id" value="{{$ogateway->id}}">
