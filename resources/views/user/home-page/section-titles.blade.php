...
@section('content')
  ...
  <div class="row">
    <div class="col-md-12">
      <div class="card">
        <form action="{{ route('user.home_page.update_section_title', ['language' => request()->input('language')]) }}" method="post">
          @csrf
              ...
              <div class="col-lg-6 offset-lg-3">
                @if ($themeInfo->theme_version == 1)
                  <div class="form-group">
                    ...
                    <input class="form-control" name="category_section_title" value="{{ empty($data->category_section_title) ? '' : $data->category_section_title }}" placeholder="Enter Category Section Title">
                  </div>
                @endif

                <div class="form-group">
                  <label>{{ __('Featured Courses Section Title') }}</label>
                  <input class="form-control" name="featured_courses_section_title" value="{{ empty($data->featured_courses_section_title) ? '' : $data->featured_courses_section_title }}" placeholder="Enter Featured Courses Section Title">
                </div>

                @if ($themeInfo->theme_version == 2)
                  <div class="form-group">
                    <label>{{ __('Featured Instructors Section Title') }}</label>
                    <input class="form-control" name="featured_instructors_section_title" value="{{ empty($data->featured_instructors_section_title) ? '' : $data->featured_instructors_section_title }}" placeholder="Enter Featured Instructors Section Title">
                  </div>
