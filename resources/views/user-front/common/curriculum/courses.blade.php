...
@section('pageHeading')
  @if (!empty($pageHeading))
    {{ $pageHeading->courses_page_title }}
  @endif
@endsection

@section('metaKeywords')
  @if (!empty($seoInfo))
    {{ $seoInfo->courses_meta_keywords }}
  @endif
@endsection

@section('metaDescription')
  @if (!empty($seoInfo))
    {{ $seoInfo->courses_meta_description }}
  @endif
@endsection

@section('content')
  @includeIf('user-front.common.partials.breadcrumb', ['breadcrumb' => $bgImg->breadcrumb, 'title' => $pageHeading->courses_page_title])
      ...
      <div class="row justify-content-center">
        <div class="col-lg-9">
              ...
              <div class="course-filter d-block align-items-center d-sm-flex">
                <select id="sort-type">
                  ...
                  <option {{ request()->input('sort') == 'ascending' ? 'selected' : '' }} value="ascending">
                    {{$keywords['price'] ?? __('Price') . ': ' }}{{__('Ascending')}}
                  </option>
                ...
                <div class="input-box">
                  ...
                  <input type="text" id="search-input" placeholder="{{ $keywords['search_course'] ?? __('Search Course') }}" value="{{ !empty(request()->input('keyword')) ? request()->input('keyword') : '' }}">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="row mt-10">
            @if (count($courses) == 0)
              <div class="col-lg-12">
                <h3 class="text-center mt-30">{{ $keywords['no_course_found'] ?? __('No Course Found') . '!' }}</h3>
              </div>
            @else
              ...
              <div class="input-radio-btn">
                <ul class="radio_common-2 radio_style2">
                  <li>
                    ...
                    <label for="radio1"><span></span>{{ $keywords['all_courses'] ?? __('All Courses') }}</label>
                  </li>
                  <li>
                    <input type="radio" {{ request()->input('type') == 'free' ? 'checked' : '' }} name="type" id="radio2" value="free">
                    <label for="radio2"><span></span>{{ $keywords['free_courses'] ?? __('Free Courses') }}</label>
                  </li>
                  <li>
                    <input type="radio" {{ request()->input('type') == 'premium' ? 'checked' : '' }} name="type" id="radio3" value="premium">
                    <label for="radio3"><span></span>{{ $keywords['premium_courses'] ?? __('Premium Courses') }}</label>
                  </li>
                </ul>
              </div>
            </div>
                ...
                <div class="input-radio-btn">
                  <ul class="radio_common-2 radio_style2">
                    <li>
                      ...
                      <label for="all-category"><span></span>{{ $keywords['all_category'] ?? __('All Category') }}</label>
                    </li>

                    @foreach ($categories as $category)
                      <li>
                        <input type="radio" {{ request()->input('category') == $category->slug ? 'checked' : '' }} name="category" id="{{ 'catRadio' . $category->id }}" value="{{ $category->slug }}">
                        <label for="{{ 'catRadio' . $category->id }}"><span></span>{{ $category->name }}</label>
                      </li>
                    @endforeach
                  </ul>
                </div>
  ...
  <form id="filtersForm" class="d-none" action="{{ route('front.user.courses', getParam()) }}" method="GET">
