    ...
    <div class="course-navigation">
      <div class="navigation-container d-flex align-items-center justify-content-between">
        <div class="course-nav-left d-flex justify-content-between align-items-center">
          <a href="{{ route('customer.my_courses', getParam()) }}" class="prev"><i class="far fa-angle-left"></i>{{ $keywords["My_Courses"] ?? __('My Courses') }}</a>
        ...
        <div class="course-nav-right">
          ...
          @endphp
          @if ($certificateStatus == 1 && (!empty($permissions) && in_array('Course Completion Certificate',$permissions)))
            <a href="{{ route('customer.my_course.get_certificate', [getParam(),'id' => request()->route('id')]) }}" class="certificate"><i class="far fa-diploma"></i>{{$keywords['certificate'] ?? __('Certificate') }}</a>
          @endif
        </div>
      </div>
    </div>
                  ...
                  <ul class="list">
                    @if (!empty($lessons))
                      @foreach ($lessons as $lesson)
                        ...
                        <li>
                          <a href="{{ route('customer.my_course.curriculum', [getParam(),'id' => request()->route('id'), 'lesson_id' => $lesson->id]) }}" class="
                            @if(request()->input('lesson_id') == $lesson->id)
                              active
                            @endif 
                            @if(Auth::guard('customer')->check() && $lesson->lesson_complete()->where('customer_id', Auth::guard('customer')->user()->id)->count() > 0) 
                              lesson-complete 
                            @endif" id="lesson-{{ $lesson->id }}">
            ...
            @if (!empty($lessonContents))
              @foreach ($lessonContents as $lessonContent)
                ...
                @switch($contentType)
                  @case('video')
                    <div class="video-box">
                      <video id="videoId{{ $lessonContent->id }}" class="video-js vjs-16-9" controls preload="none" data-setup="{}" onplay="videoCompletion(this.id, {{ $lessonContent->id }})" poster="{{ \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_LESSON_CONTENT_VIDEO_PREVIEW,$lessonContent->video_preview,$userBs,'assets/tenant/image/static/default_video_preview.jpeg') }}">
                        <source src="{{ \App\Http\Helpers\Uploader::getImageUrl(Constant::WEBSITE_LESSON_CONTENT_VIDEO,$lessonContent->video_unique_name,$userBs) }}" type="video/mp4">
                      </video>
                    </div>
                    @break
                  @case('file')
                    <div class="download-box">
                      ...
                      <form class="d-inline-block" action="{{ route('customer.my_course.curriculum.download_file', [getParam(),'id' => $lessonContent->id]) }}" method="POST">
                        @csrf
                        <button type="submit"><span><i class="fal fa-download"></i></span>{{$keywords['download'] ?? __('Download') }}</button>
                      </form>
                    </div>
                    @break
                  @case('text')
                  ...
                  @case('quiz')
                    <div class="quiz-content-box" id="quiz-content" data-content_id="{{ $lessonContent->id }}" data-completion_status="{{ $lessonContent->completion_status }}">
                      ...
                      <div id="quiz-complete" class="dis-none">
                        ...
                        <p>{{$keywords['you_scored'] ?? __('You scored') }} <span id="correct-ans-count"></span>/{{ count($quizzes) }} (<span id="result-percentage"></span>%)</p>
                        <a href="{{ url()->current() . '?lesson_id=' . request()->input('lesson_id') . '&quiz=retake' }}">{{$keywords['retake_quiz'] ?? __('Retake Quiz') }}</a>
                      </div>
                      <button class="btn btn-sm btn-info dis-none" id="check-btn">{{$keywords['check'] ?? __('Check') }}</button>
                      <button class="btn btn-sm btn-primary dis-none" id="next-btn">{{$keywords['next'] ?? __('Next') }}</button>
                    </div>
                    @break
                  @default
                    {{-- do nothing --}}
