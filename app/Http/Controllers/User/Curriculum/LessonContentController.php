...
class LessonContentController extends Controller
{
  public function contents($id, Request $request)
  {
    ...
    if ($count > $lesson_limit) {
        Session::flash('warning', 'Please delete some lessons to enable "Contents" feature.');
        return back();
    }
    ...
    $information['language'] = Language::query()->where('code', $request->language)
                                                ...
                                                ->first();
    return view('user.curriculum.lesson-content.index', $information);
  }
  ...
  {
    $data = BasicExtended::query()
        ->select('max_file_size')
        ->first();
    $maxSize = intval($data->max_file_size);
    $convertedSize = $maxSize * 1024;
