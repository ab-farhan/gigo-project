...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $information['courses'] = Course::query()
            ...
            ->join('user_instructors', 'user_course_informations.instructor_id', '=', 'user_instructors.id')
            ...
            ->select('user_courses.*', 'user_course_informations.id as courseInfoId', 'user_course_informations.title', 'user_instructors.name as instructorName', 'user_course_categories.name as category')
            ->orderByDesc('user_courses.id')
            ->get();

        $information['currencyInfo'] = $this->getUserCurrencyInfo(Auth::guard('web')->user()->id);

        return view('user.curriculum.course.index', $information);
    }
    ...
    public function create()
    {
        ...
        $currencyInfo = $this->getUserCurrencyInfo(Auth::guard('web')->user()->id);
        return view('user.curriculum.course.create', compact('languages', 'defaultLang', 'currencyInfo'));
    }
    ...
    {
        $count = LimitCheckerHelper::currentCourseCount(Auth::guard('web')->user()->id);//getting currently added course by user
        $course_limit = LimitCheckerHelper::courseLimit(Auth::guard('web')->user()->id);//course limit count of current package
        if ($count >= $course_limit) {
            Session::flash('warning', 'Course limit has been exceeded');
            return "success";
        }
    ...
    {
        $count = LimitCheckerHelper::currentFeaturedCourseCount(Auth::guard('web')->user()->id);//getting currently added featured course by user
        $featured_course_limit = LimitCheckerHelper::featuredCourseLimit(Auth::guard('web')->user()->id);//featured course limit count of current package
        if ($count >= $featured_course_limit && $request['is_featured'] == 'yes') {
            $request->session()->flash('warning', 'Featured Course Limit Exceeded!');
            return redirect()->back();
        }
        ...
        if ($request['is_featured'] == 'yes') {
            ...
            $request->session()->flash('success', 'Course removed from featured successfully!');
        }
        return redirect()->back();
    }
    ...
    public function edit($id)
    {
        ...
        $information['defaultLang'] = $languages->where('is_default', 1)->first();
        $information['currencyInfo'] = $this->getUserCurrencyInfo(Auth::guard('web')->user()->id);
        return view('user.curriculum.course.edit', $information);
    }
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        $course_limit = LimitCheckerHelper::courseLimit(Auth::guard('web')->user()->id);//course limit count of current package
        if ($count > $course_limit) {
            Session::flash('warning', 'Please delete some courses to enable "Edit" feature');
            return "success";
        }
    ...
    public function thanksPage($id)
    {
        ...
        $information['defaultLang'] = $languages->where('is_default', 1)->first();
        $information['languages'] = $languages;
        return view('user.curriculum.course.thanks-page', $information);
    }
    ...
    public function certificateSettings($id)
    {
        ...
        return view('user.curriculum.course.certificate-settings', $information);
    }
