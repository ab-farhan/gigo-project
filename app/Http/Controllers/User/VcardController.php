...
class VcardController extends Controller
{
    ...
    public function services($id) {
        $data['vcard'] = UserVcard::where('user_id', Auth::user()->id)->where('id', $id)->firstOrFail();;
        $data['services'] = $data['vcard']->user_vcard_services()->orderBy('id', 'DESC')->get();
    ...
    public function serviceStore(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $input['image'] = Uploader::upload_picture(Constant::WEBSITE_VCARD_SERVICE_IMAGE,$request->file('image'));
        }
    ...
    public function serviceUpdate(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $input['image'] = Uploader::update_picture(Constant::WEBSITE_VCARD_SERVICE_IMAGE,$request->file('image'),$service->image);
        }
    ...
    {
        $service = UserVcardService::findOrFail($request->service_id);
        if ($service->user_vcard->user_id != Auth::guard('web')->user()->id) {
            Session::flash('error', 'You are not authorized to perform this action');
            return back();
        }
        Uploader::remove(Constant::WEBSITE_VCARD_SERVICE_IMAGE,$service->image);
        $service->delete();
    ...
    public function store(Request $request) {
            ...
            'profile_image' => [
                function ($attribute, $value, $fail) use ($profileImg, $allowedExts) {
                    if (!empty($profileImg)) {
                        ...
                        if ($size > 2500000) {
                            return $fail("Image size cannot be greater than 2.5 MB");
                        }
                    }
                },
            ],
        ...
        $vcard = new UserVcard();
        $vcard->user_id = Auth::guard('web')->user()->id;
        $vcard->vcard_name = $request->vcard_name;
        $vcard->direction = $request->direction;
        $vcard->template = $request->template;
        if ($request->hasFile('profile_image')) {
            $vcard->profile_image = Uploader::upload_picture(Constant::WEBSITE_VCARD_IMAGE,$request->file('profile_image'));
        }
        if ($request->hasFile('cover_image')) {
            $vcard->cover_image = Uploader::upload_picture(Constant::WEBSITE_VCARD_IMAGE,$request->file('cover_image'));
        }
    ...
    public function update(Request $request) {
        ...
        $vcard->template = $request->template;
        if ($request->hasFile('profile_image')) {
            $vcard->profile_image = Uploader::update_picture(Constant::WEBSITE_VCARD_IMAGE,$request->file('profile_image'),$vcard->profile_image);
        }
        if ($request->hasFile('cover_image')) {
            $vcard->cover_image = Uploader::update_picture(Constant::WEBSITE_VCARD_IMAGE,$request->file('cover_image'),$vcard->cover_image);
        }
        $vcard->name = $request->name;
        $vcard->occupation = $request->occupation;
        $vcard->company = $request->company;
    ...
    public function delete(Request $request)
    {
        ...
        Uploader::remove(Constant::WEBSITE_VCARD_IMAGE, $vcard->profile_image);
        Uploader::remove(Constant::WEBSITE_VCARD_IMAGE, $vcard->cover_image);
        $vcard->delete();
    ...
    public function projectStore(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $input['image'] = Uploader::upload_picture(Constant::WEBSITE_VCARD_PROJECT_IMAGE,$request->file('image'));
        }
        $input['user_vcard_id'] = $request->user_vcard_id;
    ...
    public function projectUpdate(Request $request)
    {
        ...
        if ($project->user_vcard->user_id != Auth::guard('web')->user()->id) {
            return;
        }
        $input = $request->all();

        if ($request->hasFile('image')) {
            $input['image'] = Uploader::update_picture(Constant::WEBSITE_VCARD_PROJECT_IMAGE, $request->file('image'), $project->image);
        }
    ...
    public function projectDelete(Request $request)
    {
        ...
        Uploader::remove(Constant::WEBSITE_VCARD_PROJECT_IMAGE, $project->image);
        $project->delete();
    ...
    public function testimonials($id) {
        $data['vcard'] = UserVcard::where('user_id', Auth::guard('web')->user()->id)->where('id', $id)->firstOrFail();
        $data['testimonials'] = $data['vcard']->user_vcard_testimonials()->orderBy('id', 'DESC')->get();
    ...
    public function testimonialStore(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            $input['image'] = Uploader::upload_picture(Constant::WEBSITE_VCARD_TESTIMONIAL_IMAGE,$request->file('image'));
        }
        $input['user_vcard_id'] = $request->user_vcard_id;
    ...
    public function testimonialUpdate(Request $request)
    {
        ...
        if ($testimonial->user_vcard->user_id != Auth::guard('web')->user()->id) {
            return;
        }
        $input = $request->all();

        if ($request->hasFile('image')) {
            $input['image'] = Uploader::update_picture(Constant::WEBSITE_VCARD_TESTIMONIAL_IMAGE,$request->file('image'),$testimonial->image);
        }
    ...
    public function testimonialDelete(Request $request)
    {
        ...
        Uploader::remove(Constant::WEBSITE_VCARD_TESTIMONIAL_IMAGE, $testimonial->image);
        $testimonial->delete();
    ...
    public function colorUpdate(Request $request, $vcId) {
        ...
        if ($vcard->user_id != Auth::guard('web')->user()->id) {
            return;
        }
    ...
    public function keywords($id) {
        $vcard = UserVcard::where('user_id', Auth::guard('web')->user()->id)->where('id', $id)->firstOrFail();
        $keywords = [];
